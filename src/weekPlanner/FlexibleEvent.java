...
public class FlexibleEvent extends Event {
    ...
    private int[] getLoopBounds(ArrayList<Event> conflictable, LocalDateTime tempBegin,
    ...
    private ArrayList<Event> getConflictingEvents(ArrayList<Event> existing) {
        ...
        for(Event e : existing) {
            ...
            // if an existing event falls within the allowable range of this
            // event, add it to conflictables
            if(e.getEnd().isAfter(this.getEarliestStart())
                    && e.getStart().isBefore(this.getDueDate())
                    || e.getStart().isBefore(this.getDueDate())
                    && e.getEnd().isAfter(this.getEarliestStart()))
            {
                conflictable.add(e);
            }
        }
    ...
    private boolean findShortestGap(int loopStart, int loopEnd, ArrayList<Event> conflictable,
            LocalDateTime tempBegin, LocalDateTime tempEnd)
    {
        ...
        for(int i = loopStart; i < loopEnd; i++) {
            ...
            if(gapStart.isBefore(gapEnd) && gap.compareTo(this.getDuration()) >= 0
                    && (shortestGap == null || gap.compareTo(shortestGap) <= 0))
            {
                shortestGap = gap;
    ...
    private LocalDateTime[] getGapBounds(int gapNumber, ArrayList<Event> conflictable,
            LocalDateTime tempBegin, LocalDateTime tempEnd)
    {
